...
user_data_base = {	
    1:{
        "cc":10,"Bank":1,"name":"Harsh","balance":10000,"Transacn_Count":5,"dailyTransnLimit":40000,"ccpin":0000
    },
    2:{
        "cc":15,"Bank":1,"name":"ravi","balance":10000,"Transacn_Count":5,"dailyTransnLimit":40000,"ccpin":5645
    },
    3:{
        "cc":11,"Bank":2,"name":"Piyush","balance":10000,"Transacn_Count":5,"dailyTransnLimit":40000,"ccpin":1234
    }
}
...
bank_data_base = {
    1:{
        'Name': 'UCO Bank','ATM': [1],'user': [1,2]
    },
    2:{
        'Name': 'AXIS Bank','ATM': [2],'user': [3]
    }
}
...
def deposit():
    if user_data_base[user_id]['Transacn_Count']<1 or user_data_base[user_id]["dailyTransnLimit"]<0:
...
def admin_menu():
    ...
    if admin_data_base[admin_id]["cc"] == card and admin_data_base[admin_id]["ccpin"] == pin:
        while True:
            ...
            print('Enter 6: Update ATM Balance')
            print('Enter 7: Delete ATM')

            print('Enter 8: For deleting Users from Database')
            print('Enter 9: Update User Details')
            print('Enter 10: Update bank name')
            print('Enter 11: Delete bank')
            print('Enter 12: Withdraw all money from ATM')
...
# with draw all the money from the atm
def Withdraw_all_money_ATM():
    list_atm()
    ...
    if AtmId not in atm_data_base:
        ...
        atm_data_base[AtmId]['balance'] = 0
        print("All the money has been withdrawn!!")
...
def add_user():
    ...
    user_data_base[user_id]['Transacn_Count'] = 5
...
def add_New_bank():
    ...
    l = []
    for i in bank_data_base:
        l.append(bank_data_base[i]['Name'])
    ...
    if newBank in l:   
        print("This bank already exist!! Enter a new one")
        return
    else:
...
def update_bank_name():
    list_banks()
    while True:
        bank_id = input("Select the bank : ")
        if bank_id.isnumeric() == True:
            bank_id=int(bank_id)
            break
        else:
            print('Invalid Choice')
...
def delete_users():
    ...
    if user_id not in user_data_base:
        print("No User found")
        return
    elif user_data_base[user_id]['balance']>0:
        print("user have balance inside his account. Not possible to DELETE !")
        print("balance ", user_data_base[user_id]['balance'])
        return 
    else:
...
def Delete_ATM():
    ...
    if AtmId not in atm_data_base:
        print(f"No ATM found with ATM ID : {AtmId}")
        return
    elif atm_data_base[AtmId]['balance']>0:
        print("ATM has balance inside. Not possible to DELETE !")
        print("balance ", atm_data_base[AtmId]['balance'])
        return 
    else:
...
# Deleting the existing Bank's
def Delete_Bank():

    list_banks()
    
    while True:
        bank_id = input("Select Bank ID : ")
        if bank_id.isnumeric() == True:
            bank_id=int(bank_id)
            break
        else:
    ...
    if bank_id not in bank_data_base:
        print(f"No Bank found with ATM ID : {bank_id}")
        return
    else:
        
        for i in bank_data_base[bank_id]['user']:
            if user_data_base[i]['balance'] > 0:
                print("Bank cannot be deleted! User has cash inside account")
                return
        print("ATM and Users with the bank will be deleted")
        for i in bank_data_base[bank_id]['ATM']:
            del atm_data_base[i]
        for i in bank_data_base[bank_id]['user']:
            del user_data_base[i]
        del bank_data_base[bank_id]
